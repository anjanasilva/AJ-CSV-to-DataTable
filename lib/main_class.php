...
class main_class {
    ...
    protected function __construct() {
        add_shortcode( 'aj-csv2dt', array( $this, 'shortCode' ) ); // New shortcode.
    }
    ...
    function loadJsCssLibs() {
        ...
        wp_register_style(
            $handle = 'sorttable',
            $src    = CSV_URL . 'css/datatables.min.css'
        );
    ...
    function getCsvSource( $atts ) {
        ...
        return CSV_URL . 'us-500.csv'; // Default file URL if no .csv file source is defined.
    }
     ...
     * @return string       HTML for rendered table, or blank string if no data.
     */
    function getRenderTable( $rows, $atts ) {
        if ( 1 > count( $rows ) )
            return '';
